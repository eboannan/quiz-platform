...
const Dashboard = () => {
    ...
    const handleSaveStudent = async (e) => {
        ...
        try {
            if (editingStudentId) {
                // Update existing
                const response = await studentAPI.update(editingStudentId, newStudent);
        ...
        <div className="animate-fade-in">
            ...
            {showStudentForm && (
                <div style={{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '24px', marginBottom: '2rem', border: '1px solid #e2e8f0' }}>
                    <h3 style={{ marginBottom: '1.5rem' }}>{editingStudentId ? 'Edit Student' : 'New Student'}</h3>
                    <form onSubmit={handleSaveStudent} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                        ...
                        <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
                            <button type="button" onClick={() => { setShowStudentForm(false); setEditingStudentId(null); setNewStudent({ firstName: '', lastName: '', username: '', password: '', age: '', grade: '', accessCode: '' }); }} style={{ background: 'none', fontWeight: '600' }}>Cancel</button>
                            <button className="btn btn-primary" type="submit">{editingStudentId ? 'Update Student' : 'Save Profile'}</button>
                        </div>
                    </form>
                </div>
            )}
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px' }}>
                    <thead style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                        <tr>
                            ...
                            <th style={{ padding: '1.25rem', textAlign: 'left' }}>Password</th>
                                ...
                                <tr key={s.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                    ...
                                    <td style={{ padding: '1.25rem', textAlign: 'right' }}>
                                        <button
                                            ...
                                            style={{ color: 'var(--color-primary)', background: 'none', fontWeight: '600', marginRight: '0.5rem' }}
                                        >
                                            Edit
                                        </button>
